...
export const help = `Usage: prompton tags [options]
...
const Query = graphql(`
  query PromptonTags($offset: Int!, $limit: Int!) {
    promptonTags(offset: $offset, limit: $limit) {
      id
      name
      slug
    }
...
export default factory.createHandlers(zValidator("query", schema), async (c) => {
  ...
  return c.json(withPageURLs("tag", data.promptonTags))
})
