...
export const help = `Usage: prompton my works [options]
...
const Query = graphql(`
  query ViewerPromptonWorks($offset: Int!, $limit: Int!) {
    viewer {
      promptonUser {
...
export default factory.createHandlers(zValidator("query", schema), async (c) => {
  ...
  return c.json(withPageURLs("work", data.viewer?.promptonUser?.works ?? []))
})
