...
export const help = `Usage: prompton labels [options]
...
const Query = graphql(`
  query PromptonLabels($offset: Int!, $limit: Int!) {
    promptonLabels(offset: $offset, limit: $limit) {
      id
      name
    }
  }
...
export default factory.createHandlers(zValidator("query", schema), async (c) => {
  ...
  return c.json(withPageURLs("label", data.promptonLabels))
})
